...
def estimate_distance_from_bbox(
    ...
    geometry_mode = str(object_priors.get("geometry_mode", "mean")).lower()
    bbox_width_scale = max(float(object_priors.get("bbox_width_scale", 1.0)), 1e-6)
    bbox_height_scale = max(float(object_priors.get("bbox_height_scale", 1.0)), 1e-6)
    width_px = max(float(bbox_width_px) * bbox_width_scale, 1e-6)
    height_px = max(float(bbox_height_px) * bbox_height_scale, 1e-6)
    ...
    cues: dict[str, tuple[float, float, float]] = {}
    if width_px > 0:
        cues["width"] = (
            camera.fx_px * nominal_width / width_px,
            camera.fx_px * min_width / width_px,
            camera.fx_px * max_width / width_px,
        )
    if height_px > 0:
        cues["height"] = (
            camera.fy_px * nominal_height / height_px,
            camera.fy_px * min_height / height_px,
            camera.fy_px * max_height / height_px,
        )

    if not cues:
        return GeometryEstimate(None, None, None, notes="invalid_bbox")

    selected_cues = _select_geometry_cues(cues, geometry_mode)
    if not selected_cues:
        return GeometryEstimate(None, None, None, notes="invalid_bbox")

    nominal_values = [cue[0] for cue in selected_cues]
    min_values = [cue[1] for cue in selected_cues]
    max_values = [cue[2] for cue in selected_cues]
    return GeometryEstimate(
        nominal_m=sum(nominal_values) / len(nominal_values),
        min_m=sum(min_values) / len(min_values),
        max_m=sum(max_values) / len(max_values),
...
def _select_geometry_cues(
    cues: dict[str, tuple[float, float, float]],
    geometry_mode: str,
) -> list[tuple[float, float, float]]:
    if geometry_mode == "width_only":
        return [cues["width"]] if "width" in cues else list(cues.values())
    if geometry_mode == "height_only":
        return [cues["height"]] if "height" in cues else list(cues.values())
    if geometry_mode == "max":
        if not cues:
            return []
        return [max(cues.values(), key=lambda cue: cue[0])]
